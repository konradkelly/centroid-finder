...
public class DfsBinaryGroupFinder implements BinaryGroupFinder {
    ...
    */
    ...
    private static int[] salamanderSearch(int[][] image, int[] current, int[][] visited) {
        int startRow = current[0];
        int startCol = current[1];

        if (visited[startRow][startCol] == 1 || image[startRow][startCol] == 0) return new int[]{0, 0, 0};
        ...
        while (!stack.isEmpty()) {
            ...
            if (visited[row][col] == 1 || image[row][col] == 0) {
                continue;
            }
