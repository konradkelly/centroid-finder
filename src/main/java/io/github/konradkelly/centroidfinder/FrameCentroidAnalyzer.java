...
public class FrameCentroidAnalyzer {
    ...
    public FrameCentroidAnalyzer(int targetColor, int threshold) {
        this.binarizer = new DistanceImageBinarizer(new EuclideanColorDistance(), targetColor, threshold);
    ...
    public TimestampedCentroidResult analyze(FrameSample sample) {
        ...
        if (groups.isEmpty()) {
            return TimestampedCentroidResult.missing(sample.timestampSeconds());
        }
