...
public class DfsBinaryGroupFinderTest {
    ...
    @Test
    public void longConnectedRowDoesNotOverflowStack() {
        int width = 20_000;
        int[][] image = new int[1][width];
        ...
        assertEquals(1, groups.size());
        assertEquals(width, groups.get(0).size());
        assertEquals((width - 1) / 2, groups.get(0).centroid().x());
        assertEquals(0, groups.get(0).centroid().y());
    }
